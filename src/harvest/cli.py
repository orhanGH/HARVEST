"""Command-line interface for the modular HARVEST pipeline.

This mirrors the stage layout of ``harvest_ocr.cli`` but targets the new
``harvest.pipeline.HarvestPipeline`` orchestrator. Stage implementations are
scaffolded in Phase 1 and raise ``NotImplementedError`` until later phases
migrate the underlying logic.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="harvest",
        description="Modular historical table OCR, parsing, validation, and benchmarking.",
    )
    ...
    subparsers.add_parser(
        "layout", help="Detect table geometry and create table/row/column/cell records"
    )
    ...
    parse = subparsers.add_parser("parse", help="Normalize model output into HARVEST values")
