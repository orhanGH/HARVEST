"""Shared utilities for the modular HARVEST pipeline."""
...
def make_id(*parts: str, length: int = 12) -> str:
    """Build a stable, deterministic provenance ID from ``parts``.

    The resulting ID is a short, filesystem- and JSON-friendly hex digest of
    the parts joined with ``"/"``, prefixed by the last part for
    readability (e.g. ``page_0007_3f2a1c9de0ab``).
    """
    joined = "/".join(parts)
    digest = hashlib.sha256(joined.encode("utf-8")).hexdigest()[:length]
    prefix = safe_stem(parts[-1]) if parts else "id"
